...
product_type | String | "airline_tickets"/"train_tickets" | Тип товара, под который выдается кредит| Нет
...
<h4 id='product-schema'>Схема объекта <code>product</code> при <code>product_type == 'train_tickets'</code></h4>
...
routes   | Array | См. [схему объекта route](#route-schema) | Перелеты | Да (для train_tickets)
...
Параметр | Тип | Пример | Описание | Обязательно
--------- | --------- | --------- | --------- | ---------
product_type | String | "airline-tickets", "train_tickets" | Тип билетов | Да
